...
class dtdream_contract(models.Model):
    ...
    def get_contract_id(self):
        ...
        if name_id:
            ...
            sql_baseid = baseid + "%"
            self._cr.execute(
                    "select contract_id from dtdream_contract where contract_id like '" + sql_baseid + "' order by contract_id desc limit 1")
    ...
    def get_approvers(self):
        ...
        if config:
            ...
            self.review_ids = config.review_ids + self.applicant + self.pro_responsible_person
    ...
    required_department = fields.Many2one('hr.department', string="需求部门")
    ...
    pro_responsible_person = fields.Many2many('hr.employee', string='项目负责人')
    if_responsible_person = fields.Selection([('0', '没有'), ('1', '有')], string='是否有项目负责人')

    is_zixin_check = fields.Boolean(string='是否进行了资信调查')
    business_report = fields.Selection([('1', 'Yes'), ('2', 'No'), ('3', '不涉及')], string='是否进行商务报备')
    ...
    partner_jia = fields.Many2one('res.partner', string='签约方(甲方)',
                                  domain=['|', ('supplier', '=', True), ('customer', '=', True)])
    partner_yi = fields.Many2one('res.partner', string='签约方(乙方)',
                                 domain=['|', ('supplier', '=', True), ('customer', '=', True)])
    partner_bing = fields.Many2one('res.partner', string='签约方(其他方)',
                                   domain=['|', ('supplier', '=', True), ('customer', '=', True)])
    ...
    @api.onchange('partner_jia', 'partner_yi', 'partner_bing')
    def _compute_companies_name(self):
        if self.partner_jia and self.partner_yi and self.partner_bing:
            self.partner_companies_name = ''
            if "数梦工场".decode('utf-8') not in self.partner_jia.name:
                self.partner_companies_name = self.partner_companies_name + self.partner_jia.name + ","
            if "数梦工场".decode('utf-8') not in self.partner_yi.name:
                self.partner_companies_name = self.partner_companies_name + self.partner_yi.name + ","
            if "数梦工场".decode('utf-8') not in self.partner_bing.name:
                self.partner_companies_name = self.partner_companies_name + self.partner_bing.name

            # 去末尾逗号
            if self.partner_companies_name.endswith(","):
                self.partner_companies_name = self.partner_companies_name[:-1]

    partner_companies_name = fields.Char(string='对方公司名称', store=True)
    ...
    def _compute_approver_comments_help(self, list):
        list.reverse()
        comments = {}
        for item in list:
            if item.create_uid not in comments.keys():
                comments.__setitem__(item.create_uid, item.body)
            else:
                comments.update({item.create_uid: comments.get(item.create_uid) + item.body})
        return comments

    def _compute_approver_comments(self):
        quanqian = []
        huiqians = []
        pingshens = []

        messages = self.env['mail.message'].search([('model', '=', 'dtdream.contract'), ('res_id', '=', self.id)])
        for message in messages:
            if u'状态：权签中' in message.body:
                quanqian.append(message)
            elif u'状态：评审中' in message.body:
                pingshens.append(message)
            elif u'状态：会签中' in message.body:
                huiqians.append(message)

        pingshens_comments = self._compute_approver_comments_help(pingshens)
        huiqians_comments = self._compute_approver_comments_help(huiqians)
        quanqian_comments = ''
        quanqian.reverse()
        for item in quanqian:
            quanqian_comments = quanqian_comments + item.body

        # 以html格式写入变量
        self.approver_comments_print = "<tr><td width='25%'>评审人意见</td><td>"
        for item in pingshens_comments.keys():
            self.approver_comments_print = self.approver_comments_print + "<p>" + item.name + ":" + "</p>"
            self.approver_comments_print = self.approver_comments_print + pingshens_comments.get(item)

        self.approver_comments_print = self.approver_comments_print.encode('utf-8') + "</td></tr><tr><td>会签人意见</td><td>"
        for item in huiqians_comments.keys():
            self.approver_comments_print = self.approver_comments_print + "<p>" + item.name + ":" + "</p>"
            self.approver_comments_print = self.approver_comments_print + huiqians_comments.get(item)

        self.approver_comments_print = self.approver_comments_print.encode('utf-8') \
                                       + "</td></tr><tr><td>权签人意见</td><td><p>" \
                                       + self.quanqian_id.name.encode('utf-8') + ":</p>" \
                                       + quanqian_comments.encode('utf-8') + "</td></tr>"

    approver_comments_print = fields.Text(compute=_compute_approver_comments)

    def send_applicant_email_remind_print(self, cr, uid, id):
        damn_self = self.pool.get('dtdream.contract').browse(cr, uid, id)
        for people in damn_self.applicant:
            damn_self.env['mail.mail'].create({
                'subject': u'【dodo提醒】合同评审：合同评审申请单打印提醒',
                'body_html': u'''
                        <p>%s，您好：</p>
                        <p>您的编号为%s的合同已经会签完毕，请至dodo合同评审打印相应合同审批单，提交吴总审批后盖章。</p>
                        <p>dodo</p>
                        <p>万千业务，简单有do</p>
                        <p>%s</p>''' % (people.name, damn_self.contract_id, damn_self.write_date[:10]),
    ...
    def send_email(self, cr, uid, id):
        ...
        for people in damn_self.current_handler_ids:
            damn_self.env['mail.mail'].create({
                'subject': u'%s于%s提交合同：%s %s 评审申请，请您审批！' %
                           (damn_self.applicant.name, damn_self.create_time[:10],
                            damn_self.contract_type.name, damn_self.contract_subtype.name or ""),
                'body_html': u'''
                        <p>%s，您好：</p>
                        <p>%s提交的合同评审正等待您的审批！</p>
                        <p>合同信息如下：</p>
                        ...
                        <p>%s</p>''' % (people.name, damn_self.applicant.name, damn_self.name,
                                        damn_self.contract_id, damn_self.contract_type.name,
                                        damn_self.contract_subtype.name or "",
                                        style1, damn_self.sudo().partner_jia.name, style2,
                                        damn_self.sudo().partner_yi.name,
                                        style3, damn_self.sudo().partner_bing.name, url, url,
                                        damn_self.write_date[:10]),
                'email_from': damn_self.env['ir.mail_server'].search([], limit=1).smtp_user,
                'email_to': people.work_email,
            }).send()
        return url
    ...
    def wkf_manager_review(self):
        # 主管审批
        self.review_ids = self.review_ids + self.applicant + self.pro_responsible_person
        if not self.attachment_ids:
            raise ValidationError('请先上传附件！')
        self.message_post(body=u"合同编号：%s，提交，提交时间：%s" % (
            self.contract_id, (datetime.now() + relativedelta(hours=8)).strftime("%Y-%m-%d %H:%M:%S")))
    ...
    def wkf_countersign(self):
        # 会签中
        self.current_handler_ids = self.huiqian_ids + self.legal_interface
        self.state = '5'
        self.send_email()
    ...
    def wkf_sign(self):
        ...
        self.state = '6'
        self.send_email()
        self.send_applicant_email_remind_print()
    ...
    def message_post(self, cr, uid, thread_id, context=None, **kwargs):
        ...
        if kwargs.has_key('attachment_ids'):
            for id in kwargs['attachment_ids']:
                ...
                current_contract.att_final_name = self.pool.get('ir.attachment').browse(cr, uid, id,
                                                                                        context=context).name
                base1 = current_contract.pool.get('ir.config_parameter').get_param(current_contract.env.cr,
                                                                                   current_contract.env.user.id,
                                                                                   'web.base.url')
                ...
                for people in current_contract.current_handler_ids:
                    mess = current_contract.env['mail.mail'].create({
                        'subject': u'%s于%s提交合同：%s 评审申请已经重新上传了附件，请您审批！' %
                        ...
                        'body_html': u'''
                                ...
                                <p>万千业务，简单有do</p>
                                <p>%s</p>''' % (
                            people.name, current_contract.applicant.name, url1, url1, current_contract.write_date[:10]),
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        ...
        res = super(dtdream_contract, self).fields_view_get(view_id=view_id, view_type=view_type, toolbar=toolbar,
                                                            submenu=False)
    ...
    def btn_confirm(self):
        ...
        for people in current_record.current_handler_ids:
            current_record.write(
                    {"his_approve": [(4, self.env['hr.employee'].search([('user_id', '=', self.env.user.id)]).id)]})
        ...
        if self.temp == 'agree':
            result = u'同意'
            current_record.write({'current_handler_ids': [
                (3, self.env['hr.employee'].search([('user_id', '=', self.env.user.id)]).id)]})
            ...
            result = u'驳回'
            handlers = current_record.current_handler_ids
            current_record.signal_workflow('btn_reject')
            if len(handlers) > 1:
                people_email_list = ''
                for people in handlers:
                    if people.user_id.id != self.env.user.id:
                        people_email_list += people.work_email + ';'
                self.env['mail.mail'].create({
                    'subject': u'%s于%s提交合同：%s-%s 已被驳回！' % (
                        current_record.applicant.name, current_record.create_time[:10],
                        current_record.contract_type.name,
                        current_record.contract_subtype.name),
                    'body_html': u'''
                    <p>%s于%s提交的合同已被驳回，</p>
                    <p>合同名称：%s</p>
                    <p>合同编号：%s</p>
                    <p>dodo</p>
                    <p>万千业务，简单有do</p>
                    <p>%s</p>''' % (current_record.applicant.name, current_record.create_time[:10],
                                    current_record.name, current_record.contract_id,
                                    current_record.write_date[:10]),
        ...
        current_record.message_post(body=u"审批意见：%s，%s，状态：%s" % (result, self.reason, state_code))
...
class dtdream_contract_attachment(models.Model):
    ...
    attachment_upper = fields.Many2one('hr.employee', string='上传者', default=lambda self: self.env['hr.employee'].search(
            [('user_id', '=', self.env.user.id)]))
