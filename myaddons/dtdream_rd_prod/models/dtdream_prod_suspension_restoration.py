...
class dtdream_prod_suspension_restoration(models.Model):
    _name = "dtdream.prod.suspension.restoration"
    ...
    def compute_get_name(self):
        ...
        self.name=self.project.name+version+u'的恢复暂停'
    ...
    reason = fields.Text(string="恢复原因",track_visibility='onchange')
    ...
    role_person = fields.Many2many("res.users" ,"dtdream_suspension_rest_role_user",string="对应产品角色中的人员用户")
    his_app_user = fields.Many2many("res.users" ,"dtdream_suspension_rest_his_user",string="历史审批人用户")
    ...
    def _send_email(self,project,version,plccb):
        ...
        content = project.department.name+u"的"+project.name+u"的"+version_numb+u"恢复暂停申请，待您审批"
        base_url = project.get_base_url()
        link = '/web#id=%s&view_type=form&model=dtdream.prod.suspension.restoration' % self.id
        url = base_url+link
        ...
        if self.reason:
            ...
            if not self.version:                                           #产品
                ...
                self.message_post(body=u"""<table class="zxtable" border="1" style="border-collapse: collapse;">
                                           ...
                                           </table>""" %(project.name,u'提交恢复暂停',self.reason))
            else:
                ...
                self.message_post(body=u"""<table class="zxtable" border="1" style="border-collapse: collapse;">
                                           ...
                                           <tr><td style="padding:10px">操作</td><td style="padding:10px">%s</td></tr>
                                           <tr><td style="padding:10px">申请原因</td><td style="padding:10px">%s</td></tr>
                                           </table>""" %(project.name,version.version_numb,u'提交恢复暂停',self.reason))
        else:
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        ...
        res = super(dtdream_prod_suspension_restoration, self).fields_view_get(view_id=view_id, view_type=view_type, toolbar=toolbar, submenu=False)
    ...
    def _get_menu_id(self):
        act_windows = self.env['ir.actions.act_window'].sudo().search([('res_model', '=', 'dtdream.prod.suspension.restoration')])
        menu = None
...
class dtdream_prod_suspension_restoration_w(models.TransientModel):
    _name = "dtdream.prod.suspension.restoration.w"
    _inherit =['mail.thread']
    _description = u"恢复暂停"
    ...
    def rest_suspensiontj(self):
        if self.reason:
            ...
            if not is_plccb or not plccb:
                raise ValidationError(u"该产品没有配置PL-CCB")
            suspension = self.env['dtdream.prod.suspension.restoration'].create({'project': self.project.id, 'version': self.version.id, 'reason': self.reason, 'state': self.state})
            ...
            if not self.version:  # 产品
                ...
                suspension.message_post(body=u"""<table class="zxtable" border="1" style="border-collapse: collapse;">
                                               ...
                                               <tr><td style="padding:10px">操作</td><td style="padding:10px">%s</td></tr>
                                               <tr><td style="padding:10px">申请原因</td><td style="padding:10px">%s</td></tr>
                                               </table>""" % (project.name, u'提交恢复暂停', suspension.reason))
            else:
                ...
                project.name, version.version_numb, u'提交恢复暂停', suspension.reason))
        else:
            raise ValidationError('申请原因未填写')