...
class dtdream_execption(models.Model):
    ...
    def con_name_role(self):
        self.role_person=[(5,)]
        roles = [(4,role.person.user_id.id) for role in self.name.role_ids if role.person.id and role.person.user_id.id]
        self.write({'role_person': roles})
        roles = [role.person.user_id.id for role in self.name.role_ids if
