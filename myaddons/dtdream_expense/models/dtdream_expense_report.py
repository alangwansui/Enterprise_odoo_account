        ...
        for report in self:
            ...
            if create_hr_id==zhuguan_id_hr_employee:
                if parentdepartmentid !=False:
                    #二级主管提交给自己,从前面来的流程不可能是有二级主管的单据，因为二级主管的单据直接跳过主管审批了
                    # 这是防止驳回到主管审批
                    re_state = 'zhuguan'
                    re_currentauditperson = create_hr_id
        ...
        for report in self:
            ...
            if create_hr_id == zhuguan_id_hr_employee:
                if parentdepartmentid != False:
                    ...
                    compare_person = self.env['dtdream.expense.president'].search([('type', '=', 'zongcai')]).name.id
            else:  # 员工提交到二级主管
            ...
            elif (compare_person == no_two_auditor_hr_employee and report.total_invoicevalue <= no_two_auditor_amount) or\
                    (compare_person == no_one_auditor_hr_employee and report.total_invoicevalue <= no_one_auditor_amount):
                # 和第二审批人重复并且没超过第二审批人金额，由compare_person审批，实际上从前面的流程过来不会存在这种情况，这种情况是为驳回准备得
                re_currentauditperson = compare_person
                re_currentauditperson_userid = self.get_user_id(compare_person)
            ...
            #更新报销单
            report.write({"hasauditor": [(4, self.currentauditperson.id)]})
            report.write({'state': 'quanqianren', 'currentauditperson': re_currentauditperson,'currentauditperson_userid': re_currentauditperson_userid})
