...
class dtdream_product_line(models.Model):
    ...
    @api.one
    def _compute_is_order_pro_approveds(self):
        for pro_shenpiren in self.product_order_line_id.product_approveds:
            if self.env.user == pro_shenpiren.user_id:
                self.is_order_pro_approveds = True
                break
            else :
                self.is_order_pro_approveds = False
        for pro_shenpiren in self.product_order_line_id.business_approveds:
            if self.env.user == pro_shenpiren.user_id:
                self.is_order_bus_approveds = True
                break
            else :
                self.is_order_bus_approveds = False
        if self.product_order_line_id.state not in ('1','2','3') and self.report_is_current==True:
            self.is_order_bus_approveds = True
        self.write({'report_state':self.product_order_line_id.state})
        self.write({'report_is_current':self.product_order_line_id.is_current})

    is_order_bus_approveds = fields.Boolean(string="是否历史商务审批人",default=False,compute=_compute_is_order_pro_approveds)
    is_order_pro_approveds = fields.Boolean(string="是否历史产品审批人",default=False,compute=_compute_is_order_pro_approveds)
    product_order_line_id = fields.Many2one('dtdream.sale.order.approval',string="产品", ondelete='cascade', index=True, copy=False)

# 订单执行审批类
class dtdream_sale_order_approval(models.Model):
    _name = 'dtdream.sale.order.approval'
    _description = u"订单执行审批"
    ...
    def dtdream_send_mail(self, subject, content, email_to,appellation):
        ...
        link = '/web#id=%s&view_type=form&model=dtdream.sale.order.approval' % self.id
        url = base_url+link
        email_to = email_to
        subject = subject
        content = content
    ...
    approve_records = fields.One2many("report.handle.approve.record","order_handle_id",string="审批记录")
    ...
    def wkf_approve2(self):
        ...
        self.write({"shenpiren": [(6,0,[shenpiren.id])]})
        self.dtdream_send_mail(u"{0}于{1}提交了订单执行审批申请,请您审核产品配置!".format(self.env['hr.employee'].search([('login','=',self.create_uid.login)]).name, self.create_date[:10]),
                       u"%s提交了订单执行审批申请,等待您审核产品配置" % self.env['hr.employee'].search([('login','=',self.create_uid.login)]).name, email_to=self.shenpiren.work_email,
                       appellation = u'{0},您好：'.format(self.shenpiren.name))
    ...
    def wkf_approve4(self):
        ...
        shenpiren = self.get_shenpiren()
        self.write({"shenpiren": [(6,0,[shenpiren.id])]})
        self.dtdream_send_mail(u"{0}于{1}提交了订单执行审批申请,请您完善商务信息!".format(self.env['hr.employee'].search([('login','=',self.create_uid.login)]).name, self.create_date[:10]),
                       u"%s提交了订单执行审批申请,等待您完善商务信息" % self.env['hr.employee'].search([('login','=',self.create_uid.login)]).name, email_to=self.shenpiren.work_email,
                       appellation = u'{0},您好：'.format(self.shenpiren.name))
    ...
    def wkf_approve5(self):
        ...
        shenpiren = self.get_shenpiren()
        self.write({"shenpiren": [(6,0,[shenpiren.id])]})
        self.dtdream_send_mail(u"{0}于{1}提交了订单执行审批申请,请您审批!".format(self.env['hr.employee'].search([('login','=',self.create_uid.login)]).name, self.create_date[:10]),
                       u"%s提交了订单执行审批申请,等待您审批" % self.env['hr.employee'].search([('login','=',self.create_uid.login)]).name, email_to=self.shenpiren.work_email,
                       appellation = u'{0},您好：'.format(self.shenpiren.name))
    ...
    def _onchange_rep_pro_name(self):
        ...
        rec = self.env['dtdream.sale.business.report'].search([('project_number','=',self.rep_pro_name.project_number)])
        if len(rec) > 0:
            self.have_hardware = rec[0].have_hardware
            self.final_partner_id = rec[0].final_partner_id
            self.need_ali_grant = rec[0].need_ali_grant
            self.product_manager = rec[0].product_manager
            self.sale_business_interface_person = rec[0].sale_business_interface_person
            self.pro_background = rec[0].pro_background
            self.apply_discription = rec[0].apply_discription
            self.service_detail = rec[0].service_detail
            self.channel_discription = rec[0].channel_discription
            self.estimate_payment_condition = rec[0].estimate_payment_condition
            self.service_deliver_object = rec[0].service_deliver_object
            self.other_discription = rec[0].other_discription
            self.if_promise = rec[0].if_promise
            list_rec = []
            for rec in rec[0].product_line:
                vals = {'product_id':rec.product_id,'bom':rec.bom,'pro_type':rec.pro_type,'pro_description':rec.pro_description,'pro_name':rec.pro_name,'list_price':rec.list_price,'ref_discount':rec.ref_discount,'apply_discount':rec.apply_discount,'pro_num':rec.pro_num,'config_set':rec.config_set}
                list_rec.append(vals)
    ...
    def update_crm_data(self):
        if len(self.env['crm.lead'].search([('id','=',self.rep_pro_name.id)])) > 0:
            ...
            crm_rec.write({'system_department_id':self.system_department_id.id})
            crm_rec.write({'industry_id':self.industry_id.id})
            crm_rec.write({'office_id':self.office_id.id})
            crm_rec.write({'bidding_time':self.bidding_time})
            crm_rec.write({'pre_implementation_time':self.pre_implementation_time})
            crm_rec.write({'partner_id':self.partner_id.id})
            crm_rec.write({'supply_time':self.supply_time})
        if len(self.env['dtdream.sale.business.report'].search([('project_number','=',self.project_number)])) > 0:
            report_rec = self.env['dtdream.sale.business.report'].search([('project_number','=',self.project_number)])[0]
            report_rec.write({'system_department_id':self.system_department_id.id})
            report_rec.write({'industry_id':self.industry_id.id})
            report_rec.write({'office_id':self.office_id.id})
            report_rec.write({'bidding_time':self.bidding_time})
            ...
            report_rec.write({'need_ali_grant':self.need_ali_grant})
            report_rec.write({'product_manager':self.product_manager.id})
            report_rec.write({'sale_business_interface_person':self.sale_business_interface_person.id})
            report_rec.write({'pro_background':self.pro_background})
            report_rec.write({'apply_discription':self.apply_discription})
            report_rec.write({'service_detail':self.service_detail})
            report_rec.write({'channel_discription':self.channel_discription})
            report_rec.write({'estimate_payment_condition':self.estimate_payment_condition})
            report_rec.write({'service_deliver_object':self.service_deliver_object})
            report_rec.write({'other_discription':self.other_discription})
            report_rec.write({'apply_time':self.apply_time})
    ...
    def create(self, vals):
        ...
        result = super(dtdream_sale_order_approval, self).create(vals)
        self.env['crm.lead'].search([('id','=',result.rep_pro_name.id)]).write({'order_count':1})
        return result

    @api.multi
    def write(self, vals):
        if len(vals) == 1 and (vals.has_key('warn_text') or vals.has_key('state')):
            return super(dtdream_sale_order_approval, self).write(vals)
        ...
        if vals.has_key('rep_pro_name'):
            vals['project_number'] = self.env['crm.lead'].search([('id','=',vals['rep_pro_name'])])[0].project_number
        result = super(dtdream_sale_order_approval, self).write(vals)
        return result

    @api.multi
    def unlink(self):
        if (self.state == "0" and self.create_uid.id == self._uid) or self.env.user.login=="admin":
            self.env['crm.lead'].search([('id','=',self.rep_pro_name.id)]).write({'order_count':0})
            return super(dtdream_sale_order_approval, self).unlink()
        else :
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        cr = self.env["dtdream.sale.order.approval"].search([("rep_pro_name.id", "=", self.env.context.get('active_id'))])
        res = super(dtdream_sale_order_approval, self).fields_view_get(view_id=view_id, view_type=view_type, toolbar=toolbar, submenu=False)
        if res['type'] == "form":
            if len(cr):
...
class dtdream_crm_lead(models.Model):
    ...
    order_count = fields.Integer(string="商务报备单据数量",default=0,readonly=True)

    @api.multi
    def action_dtdream_sale_order(self):
...
class report_handle_approve_record(models.Model):
    _inherit = "report.handle.approve.record"
    _order = "id desc"

    order_handle_id = fields.Many2one("dtdream.sale.order.approval",string="订单执行审批")