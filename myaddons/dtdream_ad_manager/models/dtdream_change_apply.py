...
class dtdream_change_apply(models.Model):
    ...
    def accept_refuse_mail(self):
        action = self.env['ir.model.data'].search([('name', '=', 'act_change_apply')]).res_id
    ...
    def user_change_mail(self, gid):
        ...
        menu = self.env['ir.model.data'].search([('name', '=', 'menu_ad')]).res_id
        url = self.get_base_url() + ('/web?#id=%s&view_type=form&model=dtdream.change.apply&action=%s&menu_id=%s' % (gid, action, menu))
        email_list = ''
    ...
    def implement_mail(self, email_to):
        action = self.env['ir.model.data'].search([('name', '=', 'act_change_implement')]).res_id
        menu = self.env['ir.model.data'].search([('name', '=', 'menu_ad')]).res_id
        url = self.get_base_url() + ('/web?#id=%s&view_type=form&model=dtdream.ad.group&action=%s&menu_id=%s' % (self.id, action, menu))
        person = ''
        for user in self.users:
            person += (user.nick_name + '<br/>')
        self.env['mail.mail'].create({
            'body_html': u'''<p>您好，有待实施的域群组变化申请已提交，请前往查看。</p>
                             <p><a href="%s">%s</a></p>
                             <table border="1">
                                <tr>
                                    <th>群组名称</th>
